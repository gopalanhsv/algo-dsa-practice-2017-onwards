...
public:
    ...
    minSteps(string s, string t) {
        ...
        // building different frequency count tables for 's' & 't' and totalling up the
        // excess difference in frequency counts of letters in 't' compared to 's';
        ...
        // and adding up the letter counts with -ve counts (these are the excess letters in
        // 't' compared to 's' which must be replaced)
        ...
        for (auto i = 0; i != 26; ++i) {
            if (freqCntTbl[i] < 0) {
                // Roundabout technique of getting absolute/magnitude of diffs
                nSteps -= freqCntTbl[i];
            }
        }
        return nSteps;
